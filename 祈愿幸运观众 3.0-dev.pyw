...
class MovableWindowWidget(QWidget):
    def __init__(self, parent=None):
        super(MovableWindowWidget, self).__init__(parent)
...
class AboutWindow(MovableWindowWidget):
    def __init__(self, parent=None):
...
class LogWindow(MovableWindowWidget):
    def __init__(self, parent=None):
...
class SettingsWindow(MovableWindowWidget):
...
class WishWindow(MovableWindowWidget):
    def __init__(self, parent=None):
        ...
        self.set_style(self.information_button, 'gray', 'black',
                       150, 26, '祈愿详情')
        ...
        self.information = QLabel(self.information_list[0], self)
        ...
        self.label.setFont(QFont(_global_font, 19))
    ...
    def draw_once(self):
        self.clear_label()
        self.label.setText(f'{self.get_lucky()}')
    ...
    def update_label(self):
        if self.update_label_index < len(self.numbers):
            self.label.setText(' '.join(f'{num}' for num in self.numbers[:self.update_label_index + 1]))
